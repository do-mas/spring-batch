...
import exc.support.rowset.DefaultRowSetFactory;
import exc.support.rowset.RowSet;
import exc.support.rowset.RowSetFactory;
...
public abstract class AbstractExcelItemReader<T> extends AbstractItemCountingItemStreamItemReader<T> implements
    ...
    protected T doRead() throws Exception {
        ...
        if (rs.next()) {
            System.out.println("current row " + rs.getCurrentRow());
...
//            return null;
            ...
            this.currentSheet++;
            System.out.println("current sheet " + this.currentSheet);
    ...
    protected void doOpen() throws Exception {
        ...
        if (!this.resource.exists()) {
            if (this.strict) {
                throw new IllegalStateException("Input resource must exist (csvReader is in 'strict' mode): "
                        + this.resource);
            }
        ...
        if (!this.resource.isReadable()) {
            if (this.strict) {
                throw new IllegalStateException("Input resource must be readable (csvReader is in 'strict' mode): "
                        + this.resource);
            }
    ...
    /**
     * In strict mode the csvReader will throw an exception on
